...
export async function initiateMpesaCharge(phone: string, amountKes: number, reference: string) {
    ...
    if (data.data?.status === 'send_otp' || data.message === 'Charge attempted')
...
export function verifyWebhookSignature(body: string, signature: string): boolean {
    return crypto.createHmac('sha512', PAYSTACK_SECRET).update(body).digest('hex') === signature;
}

export function generateToken(): string { return String(Math.floor(1000 + Math.random() * 9000)); }
